<template>
  <div class="addPage">
    ...
    <form class="form">
      ...
      <router-link :to="{ name: 'LessonsList' }" @click="setLessonLocalStorage" class="returnBack"
...
const teachers = JSON.parse(localStorage.getItem('users') as string).filter(
  (elem: any) => elem.role === 'Учитель'
)
const students = JSON.parse(localStorage.getItem('users') as string).filter(
  (elem: any) => elem.role === 'Ученик'
)
let arrLessons = []
let lesson = {
